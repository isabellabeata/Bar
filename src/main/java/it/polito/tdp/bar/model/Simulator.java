...
public class Simulator{
	...
	private void processaEvento(Event e) {
		...
		case ARRIVO_GRUPPO_CLIENTI:
			...
			this.statistiche.incrementaClienti(e.getPersone());
			
			//cerco un tavolo
			Tavolo tavolo= null;
			
			for(Tavolo t: this.tavoli) {
				if(!t.isOccupato()&& t.getPosti()>=e.getPersone() && t.getPosti()*this.OCCUPAZIONE_MAX<=e.getPersone()) {
					tavolo=t;
					break;
				}
			}
			if(tavolo!=null) {
				System.out.format("Trovato un tavolo da %d per %d persone", tavolo.getPosti(), e.getPersone());
				statistiche.incrementaSoddisfatti(e.getPersone());
				...
				queue.add(new Event(EventType.TAVOLO_LIBERATO, e.getTime().plus(e.getDurata()),
						e.getPersone(),e.getDurata(), e.getTolleranza(), tavolo));
			}else {
				//c'è solo il bancone
				double bancone= Math.random();
				if(bancone<=e.getTolleranza()) {
					//sì, ci fermiamo al bancone
					System.out.format("%d persone si fermano al bancone ", e.getPersone());
					statistiche.incrementaSoddisfatti(e.getPersone());
				}else {
					//no andiamo a casa
					statistiche.incrementaInsoddisfatti(e.getPersone());
					System.out.format("%d persone vanno a casa ", e.getPersone());
				}
			}
